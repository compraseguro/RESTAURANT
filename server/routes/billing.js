...
router.get('/config', authenticateToken, requireRole('admin'), (req, res) => {
  ...
  res.json({
    ...
    billing_provider: restaurant.billing_provider || 'nubefact',
    billing_api_url: restaurant.billing_api_url || '',
    billing_api_token: '',
    has_billing_api_token: Boolean((restaurant.billing_api_token || '').trim()),
    billing_series_boleta: restaurant.billing_series_boleta || 'B001',
    billing_series_factura: restaurant.billing_series_factura || 'F001',
...
router.put('/config', authenticateToken, requireRole('admin'), (req, res) => {
  try {
    ...
    runSql(
      ...
      WHERE id = ?`,
      [
        String(company_ruc || '').trim(),
        String(legal_name || '').trim(),
        Number(billing_enabled ? 1 : 0),
        String(billing_provider || 'nubefact').trim() || 'nubefact',
        String(billing_api_url || '').trim(),
        nextToken,
        normalizeSeries(billing_series_boleta, 'B001'),
        normalizeSeries(billing_series_factura, 'F001'),
    ...
    res.json({
      ...
      billing_enabled: Number(updated.billing_enabled || 0),
      billing_provider: updated.billing_provider || 'nubefact',
      billing_api_url: updated.billing_api_url || '',
      billing_api_token: '',
      has_billing_api_token: Boolean((updated.billing_api_token || '').trim()),
      billing_series_boleta: updated.billing_series_boleta || 'B001',
      billing_series_factura: updated.billing_series_factura || 'F001',
